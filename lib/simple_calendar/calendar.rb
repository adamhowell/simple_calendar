...
module SimpleCalendar
  class Calendar
    ...
    def url_for_next_view
      view_context.url_for(@params.except(:done).merge(start_date_param => (date_range.last + 1.day).iso8601))
    end

    def url_for_previous_view
      view_context.url_for(@params.except(:done).merge(start_date_param => (date_range.first - 1.day).iso8601))
    end
